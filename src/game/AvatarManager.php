...
class AvatarManager
{
    ...
    public function getAvatarById(int $avatarId): ?Avatar
    {
        foreach ($this->avatars as $avatar) {
            if ($avatar->getAvatarInfo()->getAvatarId() === $avatarId) {
                return $avatar;
            }
        }
        ...
        $sceneTeamUpdateNotify->setSceneTeamAvatarList(
            array_map(function (int $guid): SceneTeamAvatar {
                ...
                return $avatar->getSceneTeamAvatar(
                    $guid === $this->getCurAvatarGuid()
                );
            }, $curTeam)
        );

        $this->session->send(
            new DataPacket('AvatarTeamUpdateNotify', $avatarTeamUpdateNotify)
        );
        $this->session->send(
            new DataPacket('SceneTeamUpdateNotify', $sceneTeamUpdateNotify)
        );
    }
