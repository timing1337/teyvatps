...
use AbilityControlBlock;
use AbilityEmbryo;
use AbilitySyncStateInfo;
use AvatarEnterSceneInfo;
use AvatarInfo;
use FightPropPair;
use PropPair;
use PropValue;
...
class Avatar extends Entity
{
    ...
    public function getSceneEntityInfo(): SceneEntityInfo
    {
        ...
        $sceneEntityInfo->setPropList([
            $this->getAvatarPropPair(EntityProperties::PROP_LEVEL),
        ]);
        $fightProp = [];
    ...
    public function getSceneAvatarInfo(): SceneAvatarInfo
    {
        ...
        $sceneAvatarInfo->setInherentProudSkillList(
            $this->avatarInfo->getInherentProudSkillList()
        );
        $sceneAvatarInfo->setProudSkillExtraLevelMap(
            $this->avatarInfo->getProudSkillExtraLevelMap()
        );
        $sceneAvatarInfo->setSkillLevelMap(
            $this->avatarInfo->getSkillLevelMap()
        );
        $sceneAvatarInfo->setTalentIdList($this->avatarInfo->getTalentIdList());
        $sceneAvatarInfo->setCoreProudSkillLevel(
            $this->avatarInfo->getCoreProudSkillLevel()
        );
