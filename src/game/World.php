...
class World
{
    ...
    public function __construct(Session $session)
    {
        ...
        $this->mpLevelEntityId = $this->getNextEntityId(\ProtEntityType::PROT_ENTITY_TYPE_MP_LEVEL);
    }

    public function getWorldLevel(): int
    {
        $level = $this->session->getPlayer()->getProp(DataProperties::PROP_PLAYER_LEVEL);
        return match (true) {
            $level <= 20 => 1,
            $level <= 25 => 2,
            $level <= 30 => 3,
    ...
    {
        return (PlayerData::UID << 32) + ++$this->nextGuid;
    }
    ...
    public function addEntity(Entity $entity): void
    {
        ...
        $this->session->send(new DataPacket('SceneEntityAppearNotify', $packet));
