...
use ChangeAvatarReq;
use ChangeAvatarRsp;
use ChooseCurAvatarTeamReq;
use ChooseCurAvatarTeamRsp;
use SetUpAvatarTeamReq;
...
class TeamManager
{
    public static function init(): void
    {
        NetworkServer::registerProcessor(
            SetUpAvatarTeamReq::class,
            function (
                Session $session,
                SetUpAvatarTeamReq $request
            ): SetUpAvatarTeamRsp {
                ...
                $session->getPlayer()->getAvatarManager()->setTeam(
                    $request->getTeamId(),
                    $team
                );
                if ($request->getCurAvatarGuid() !== $session->getPlayer()
                        ->getAvatarManager()->getCurAvatarGuid()
                ) {
        ...
        NetworkServer::registerProcessor(
            ChangeAvatarReq::class,
            function (
                Session $session,
                ChangeAvatarReq $request
            ): ChangeAvatarRsp {
                $session->getPlayer()->getAvatarManager()->setCurAvatarGuid(
                    $request->getGuid()
                );
        ...
        NetworkServer::registerProcessor(
            ...
            function (
                ...
                $session->getPlayer()->getAvatarManager()->setCurAvatarGuid(
                    $session->getPlayer()->getAvatarManager()->getTeam(
                        $req->getTeamId()
                    )[0]
                );
