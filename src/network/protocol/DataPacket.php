...
class DataPacket
{
    ...
    public function __construct(string|int $id = null, Message $data = null, PacketHead $header = null)
    {
        if ($header === null) {
    ...
    public function decode(Buffer $buffer): bool
    {
        try {
            ...
            if ($buffer->getLength() !== self::HEADER_LENGTH + $metadataSize + $dataSize) {
                return false;
            }
            $this->header = new PacketHead();
            $this->header->mergeFromString($buffer->slice(10, 10 + $metadataSize)->toString());
            $name = $this->getName();
            ...
            $this->data = new ($this->getName())();
            $this->data->mergeFromString($buffer->slice(10 + $metadataSize,
                10 + $metadataSize + $dataSize)->toString());
    ...
    {
        if (!$this->data instanceof Message || !$this->header instanceof PacketHead) {
            throw new Exception("Packet " . $this->pid() . " is not properly initialized");
        }

        $header = Buffer::new($this->header->serializeToString());
        $data = Buffer::new($this->data->serializeToString());

        $buffer = Buffer::allocate(self::HEADER_LENGTH + $header->getLength() + $data->getLength());
        ...
        $buffer->writeUInt16BE(self::MAGIC_END, 10 + $header->getLength() + $data->getLength());
        return $buffer;
    }
}